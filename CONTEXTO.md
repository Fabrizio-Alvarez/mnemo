...
- `quiz` (v1 ✅): opción múltiple derivada — distractores **autorales** (`### distractores`: plausibles pero falsos del mismo tema) priorizados, hermanas de fallback (`armarQuiz` en el dominio). Modo práctica: no escribe en la BD.
- `kata` (2026-08-31 ✅): tarjeta de ejercicio de código (`### kata`: firma + tests JSON). Editor en la sesión de estudio, tests corren en el navegador (eval client-side), y los mismos 4 botones SM-2 califican — la repetición espaciada aplicada a *reescribir* el algoritmo. Para conocimiento procedural (algoritmos/ED): se aceita implementando, no recordando.
- `cloze` (2026-09-03 ✅): `==hueco==` en la respuesta (verbatim en BD, render puro). En sesión: la respuesta con huecos ES la pregunta — blanks ocultos, texto recordado resaltado al revelar, mismo SM-2. En quiz: fill-in (enunciado = respuesta con el primer hueco en blank, correcta = texto del hueco, distractores autorales = rellenos erróneos).
...
- **PrismaNeonHTTP no soporta `$transaction`:** rechaza `startTransaction` con "Transactions are not supported in HTTP mode" — cada query es un fetch separado, sin atomicidad. Síntoma: los botones de calificar "no hacían nada" (la action explotaba antes de los setState). Fix: operaciones secuenciales con orden auto-reparable — `calificar` crea el log ANTES de actualizar el card (si el update falla, el log existe y undo limpia); `deshacerUltima` restaura el card ANTES de borrar el log (re-undo idempotente). Local (PrismaPg) pierde la atomicidad pero gana el mismo comportamiento de auto-reparación.
...
- ✅ **Modo kata** (2026-08-31): ejercicios de código en la sesión de estudio (editor + tests client-side + SM-2) · mazo "Algoritmos y Estructuras de Datos" (23 tarjetas: 15 conceptuales + 8 katas, soluciones de referencia verificadas 38/38) · dominio 71/71.
- ✅ **Modo cloze + distractores masivos** (2026-09-03): cloze en sesión (blanks/revelado resaltado) y quiz fill-in · `### distractores` en los 7 mazos de entrevista (236 secciones nuevas vía 8 agentes paralelos — 271/271 tarjetas conceptuales con distractores autorales) · 5 clozes en SQL · fix: atajos de teclado ignoran elementos editables (el espacio del editor kata revelaba la solución) · dominio 78/78.
...
- ✅ **Móvil-first + PWA instalable** (2026-08-28): sesión de estudio llena el viewport (`flex min-h-[calc(100dvh-11.5rem)]`), botones de calificación abajo en zona del pulgar (50px, `pb-[env(safe-area-inset-bottom)]` con `viewport-fit=cover`), hints de teclado solo con `[@media(hover:hover)]`, nav 44px. Instalable: `manifest.webmanifest` + iconos generados programaticamente (rombo violeta, PNG encoder mínimo en Python): `icon-192/512`, `apple-touch-icon`, `icon-maskable-512`. Sin service worker ni offline — el diseño difiere local-first hasta que el uso lo pida.
